...
echo -e "\e[0;31m.-----------------------------------------.${NC}"
echo -e "\e[0;31m|      \e[0;36mCHANGE PORT WEBSOCKET OPENSSH\e[m      \e[0;31m|${NC}"
echo -e "\e[0;31m'-----------------------------------------'${NC}"
echo -e " ${RED}>>${NC}\e[1;33mChange Port For SSH & OVPN WS:${NC}"
...
echo -e "${RED}Port Websocket SSH(HTTP) $vpn is used${NC}"
fi
;;
2)
...
echo -e "${RED}Port Websocket SSL(HTTPS) $vpn is used${NC}"
fi
;;
3)
...
echo -e "${RED}Port Ovpn Websocket $vpn is used${NC}"
fi
;;
x)
clear
